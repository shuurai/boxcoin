Lifecoin Core version *0.15.1* is now available from:

  <https://download.lifecoin.org/lifecoin-0.15.1/>
...
A number of changes to the way Lifecoin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 lifes per kB (0.00001 WLC). This is to prepare for dropping the minimum transaction fee to 0.1 lifes per kB (0.0001 WLC) in the next release.
