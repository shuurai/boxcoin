Lifecoin Core version 0.13.2 is now available from:

  <https://download.lifecoin.org/lifecoin-0.13.2.1/>
...
updates will be released anymore. Without security updates, using a lifecoin
...
ECDSA signatures inside Lifecoin transactions now use validation using
...
Previous versions of Lifecoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Lifecoin Core was to raise the default minimum
relay fee.

Lifecoin Core 0.13.2 will have a strict maximum size on the mempool. The
...
Lifecoin Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running lifecoind without having to do any manual configuration.
...
Lifecoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Lifecoin Core will relay transactions with
...
entirely in Lifecoin Core 0.13, and it is at this time undecided whether the
...
Lifecoin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Lifecoin Core automatically creates a hidden service to listen on, without
manual configuration. Lifecoin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Lifecoin Core is listening, and
...
Lifecoind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Lifecoin
Core to use floating fees. By default, Lifecoin Core will use floating
fees.
...
At all times, Lifecoin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) WLC.
Furthermore, Lifecoin Core will never create transactions paying less than
...
`lifecoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
Then, to tunnel a SSL connection on 29663 to a RPC server bound on localhost on port 19773 do:

    stunnel -d 29663 -r 127.0.0.1:19773 -p stunnel.pem -P ''
...
caching. A sample config for apache2 could look like:
    ...
    <Location /lifecoinrpc>
        ProxyPass http://127.0.0.1:9773/
        ProxyPassReverse http://127.0.0.1:9773/
        # optional enable digest auth
        # AuthType Digest
        # ...

        # optional bypass lifecoind rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< lifecoinrpc:<password>
    </Location>
...
Restarting lifecoind will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in lifecoin.conf
...
lifecoin-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/lifecoin-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Lifecoin Core code base has
...
- `lifecoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `lifecoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the Lifecoin scripting system that in combination with
...
Lifecoin transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of Lifecoin Core.
...
The two phases are distinct in the Lifecoin-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Lifecoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Lifecoin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Lifecoin block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of lifecoins they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of lifecoins being spent is exactly
...
- **Increased security for multisig:** Lifecoin addresses (both P2PKH addresses
  ...
  believed to be provided by Lifecoin's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Lifecoin's current ledger (called the UTXO set),
  ...
  note that Lifecoin Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Lifecoin users to individually opt-in to almost any change in the Lifecoin
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and Lifecoin Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the Lifecoin
client to Lifecoin Core 0.13.2 to signal for segwit support. When a
...
Since Lifecoin Core 0.10.0, nodes have defaulted to only relaying and
...
- `lifecoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `lifecoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
