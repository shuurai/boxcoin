Boxcoin Core version *0.15.1* is now available from:

  <https://download.boxcoin.org/boxcoin-0.15.1/>
...
A number of changes to the way Boxcoin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 boxes per kB (0.00001 OMB). This is to prepare for dropping the minimum transaction fee to 0.1 boxes per kB (0.0001 OMB) in the next release.
